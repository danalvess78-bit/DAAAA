...
export const api = {
  ...
  addInstructorCategory: async (category: string, minRank?: string) => {
    const res = await fetch("/api/instructor-categories", {
      method: "POST",
      headers: getAuthHeaders(),
      body: JSON.stringify({ category, minRank })
  ...
  addAmanCategory: async (category: string, minRank?: string) => {
    const res = await fetch("/api/aman-categories", {
      method: "POST",
      headers: getAuthHeaders(),
      body: JSON.stringify({ category, minRank })
  ...
  addEsaoCategory: async (category: string, minRank?: string) => {
    const res = await fetch("/api/esao-categories", {
      method: "POST",
      headers: getAuthHeaders(),
      body: JSON.stringify({ category, minRank })
  ...
  applyWarning: async (targetNick: string, reason: string, quantity?: number) => {
    const res = await fetch("/api/users/warn", {
      ...
      body: JSON.stringify({ targetNick, reason, quantity })
    });
    if (!res.ok) {
