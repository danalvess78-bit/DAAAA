...
interface DBStructure {
  ...
  instructorCategoriesConfig?: Record<string, { minRank?: MilitaryRank }>;
  amanCategories?: string[];
  amanCategoriesConfig?: Record<string, { minRank?: MilitaryRank }>;
  esaoCategories?: string[];
  esaoCategoriesConfig?: Record<string, { minRank?: MilitaryRank }>;
  customPermissions?: {
...
// Store pending deletions to replicate them in Supabase
let pendingDeletions: { table: string; id: string }[] = [];
...
export async function syncIndividualTables(supabase: any) {
  try {
    ...
    if (pendingDeletions.length > 0) {
      console.log(`[SUPABASE] Processando ${pendingDeletions.length} exclusões pendentes...`);
      const deletionsToProcess = [...pendingDeletions];
      for (const del of deletionsToProcess) {
        try {
          const { error } = await supabase.from(del.table).delete().eq("id", del.id);
          if (!error) {
            // Remove from pendingDeletions list
            pendingDeletions = pendingDeletions.filter(x => !(x.table === del.table && x.id === del.id));
            console.log(`[SUPABASE] Excluído id ${del.id} da tabela ${del.table} com sucesso.`);
          } else {
            console.warn(`[SUPABASE] Erro ao excluir id ${del.id} da tabela ${del.table}:`, error.message);
          }
        } catch (e: any) {
          console.warn(`[SUPABASE] Exceção ao excluir id ${del.id} da tabela ${del.table}:`, e.message);
        }
      }
    }
...
export function ensureRankConfigsAndDocumentsExist() {
  ...
  let categoriesNeededSave = false;
  if (db.instructorCategories === undefined || !Array.isArray(db.instructorCategories)) {
    db.instructorCategories = [];
    categoriesNeededSave = true;
  }
  if (!db.instructorCategoriesConfig) {
    db.instructorCategoriesConfig = {};
    categoriesNeededSave = true;
  }

  if (db.amanCategories === undefined || !Array.isArray(db.amanCategories)) {
    db.amanCategories = [];
    categoriesNeededSave = true;
  }
  if (!db.amanCategoriesConfig) {
    db.amanCategoriesConfig = {};
    categoriesNeededSave = true;
  }

  if (db.esaoCategories === undefined || !Array.isArray(db.esaoCategories)) {
    db.esaoCategories = [];
    categoriesNeededSave = true;
  }
  if (!db.esaoCategoriesConfig) {
    db.esaoCategoriesConfig = {};
    categoriesNeededSave = true;
  }

  if (categoriesNeededSave) {
    console.log("[CATEGORIES AUTO-SEED] Inicializando categorias padrão em memória e salvando no Supabase...");
    saveDB();
  }
}
...
export const dbOperations = {
  ...
  deleteMission: (adminId: string, missionId: string) => {
    ...
    pendingDeletions.push({ table: "fmb_missions", id: missionId });
    saveDB();
  },
  ...
  createDocument: (
    ...
    const hasPermission = user.role === MilitaryRank.ADMSUPREMO || dbOperations.hasUserPermission(userId, "canManageDocs");
  ...
  updateDocument: (
    ...
    const hasPermission = user.role === MilitaryRank.ADMSUPREMO || dbOperations.hasUserPermission(userId, "canManageDocs") || doc.authorNick === user.habboNick;
  ...
  deleteDocument: (userId: string, docId: string): void => {
    ...
    pendingDeletions.push({ table: "fmb_documents", id: docId });
  ...
  deleteTraining: (instructorId: string, trainingId: string): void => {
    ...
    pendingDeletions.push({ table: "fmb_trainings", id: trainingId });
  ...
  deleteRecruitLesson: (instructorId: string, id: string): void => {
    ...
    pendingDeletions.push({ table: "fmb_recruit_lessons", id });
    dbOperations.addLog(
      instructorId, 
      user.habboNick, 
      "EXCLUIR_AULA", 
  ...
  getInstructorCategoriesWithRanks: () => {
    const list = dbOperations.getInstructorCategories();
    if (!db.instructorCategoriesConfig) {
      db.instructorCategoriesConfig = {};
    }
    return list.map(name => ({
      name,
      minRank: db.instructorCategoriesConfig?.[name]?.minRank || null
    }));
  },

  addInstructorCategory: (category: string, minRank?: MilitaryRank) => {
    if (!db.instructorCategories) {
      db.instructorCategories = [];
    }
    const trimmed = category.trim();
    ...
    if (minRank) {
      db.instructorCategoriesConfig[trimmed] = { minRank };
    }

    saveDB();
    return dbOperations.getInstructorCategoriesWithRanks();
  },
  ...
  deleteInstructorCategory: (category: string) => {
    ...
    const trimmed = category.trim();
    db.instructorCategories = db.instructorCategories.filter(c => c.toLowerCase() !== trimmed.toLowerCase());

    if (db.instructorCategoriesConfig) {
      const exactKey = Object.keys(db.instructorCategoriesConfig).find(k => k.toLowerCase() === trimmed.toLowerCase());
      if (exactKey) {
        delete db.instructorCategoriesConfig[exactKey];
      }
    }
  ...
  getAmanCategoriesWithRanks: () => {
    const list = dbOperations.getAmanCategories();
    if (!db.amanCategoriesConfig) {
      db.amanCategoriesConfig = {};
    }
    return list.map(name => ({
      name,
      minRank: db.amanCategoriesConfig?.[name]?.minRank || null
    }));
  },

  addAmanCategory: (category: string, minRank?: MilitaryRank) => {
    if (!db.amanCategories) {
      db.amanCategories = [];
    }
    const trimmed = category.trim();
    ...
    if (minRank) {
      db.amanCategoriesConfig[trimmed] = { minRank };
    }

    saveDB();
    return dbOperations.getAmanCategoriesWithRanks();
  },
  ...
  deleteAmanCategory: (category: string) => {
    ...
    const trimmed = category.trim();
    db.amanCategories = db.amanCategories.filter(c => c.toLowerCase() !== trimmed.toLowerCase());

    if (db.amanCategoriesConfig) {
      const exactKey = Object.keys(db.amanCategoriesConfig).find(k => k.toLowerCase() === trimmed.toLowerCase());
      if (exactKey) {
        delete db.amanCategoriesConfig[exactKey];
      }
    }
  ...
  getEsaoCategoriesWithRanks: () => {
    const list = dbOperations.getEsaoCategories();
    if (!db.esaoCategoriesConfig) {
      db.esaoCategoriesConfig = {};
    }
    return list.map(name => ({
      name,
      minRank: db.esaoCategoriesConfig?.[name]?.minRank || null
    }));
  },

  addEsaoCategory: (category: string, minRank?: MilitaryRank) => {
    if (!db.esaoCategories) {
      db.esaoCategories = [];
    }
    const trimmed = category.trim();
    ...
    if (minRank) {
      db.esaoCategoriesConfig[trimmed] = { minRank };
    }

    saveDB();
    return dbOperations.getEsaoCategoriesWithRanks();
  },
  ...
  deleteEsaoCategory: (category: string) => {
    ...
    const trimmed = category.trim();
    db.esaoCategories = db.esaoCategories.filter(c => c.toLowerCase() !== trimmed.toLowerCase());

    if (db.esaoCategoriesConfig) {
      const exactKey = Object.keys(db.esaoCategoriesConfig).find(k => k.toLowerCase() === trimmed.toLowerCase());
      if (exactKey) {
        delete db.esaoCategoriesConfig[exactKey];
      }
    }
