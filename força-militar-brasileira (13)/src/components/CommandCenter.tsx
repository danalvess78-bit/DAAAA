...
export default function CommandCenter({ user, onLogout, onUpdateMe }: CommandCenterProps) {
  ...
  // New Warning Post states
  const [warnTargetNick, setWarnTargetNick] = useState("");
  const [warnReason, setWarnReason] = useState("");
  const [warnQuantity, setWarnQuantity] = useState(1);
  ...
  const handleWarningSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!warnTargetNick.trim() || !warnReason.trim()) {
      setWarnError("Digite o nick do militar e insira a justificativa.");
      return;
    }
    ...
    setWarnSuccess(null);
    try {
      await api.applyWarning(warnTargetNick.trim(), warnReason.trim(), Number(warnQuantity));
      setWarnSuccess(`Advertência (+${warnQuantity}) aplicada ao perfil de @${warnTargetNick.trim()}!`);
      setWarnTargetNick("");
      setWarnReason("");
      setWarnQuantity(1);
      fetchWarningLogs();
      ...
      setWarnError(err.message || "Erro ao processar aplicação de advertência.");
    } finally {
                ...
                >
                  ...
                  {operacionalSubTab === "status" ? (
                    ...
                    <div className="space-y-4">
                      ...
                      {/* FORM TO POST WARNINGS */}
                      {(userPermissions?.canWarn || user.role === MilitaryRank.ADMSUPREMO) && (
                        <div className="bg-fmb-slate/40 border border-red-900/35 rounded-lg p-5 shadow-lg">
                          <h4 className="text-xs font-mono font-black uppercase text-red-400 tracking-wider mb-3 flex items-center space-x-1.5 border-b border-fmb-army/15 pb-2">
                            <AlertTriangle className="w-4 h-4 text-red-500" />
                            <span>Emitir e Registrar Advertência Disciplinar</span>
                          </h4>

                          {warnSuccess && (
                            <div className="p-3 mb-3 bg-green-950/20 border border-green-500/35 text-green-400 rounded font-mono text-xs text-left">
                              {warnSuccess}
                            </div>
                          )}
                          {warnError && (
                            <div className="p-3 mb-3 bg-red-950/25 border border-red-500/35 text-red-300 rounded font-mono text-xs text-left">
                              {warnError}
                            </div>
                          )}

                          <form onSubmit={handleWarningSubmit} className="space-y-4 text-left">
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                              <div className="md:col-span-1">
                                <label className="text-[10px] uppercase text-fmb-gold block mb-1">Nick do Infrator</label>
                                <div className="relative">
                                  <span className="absolute left-2.5 top-2 text-gray-500 font-mono text-xs">@</span>
                                  <input
                                    type="text"
                                    value={warnTargetNick}
                                    onChange={(e) => setWarnTargetNick(e.target.value)}
                                    placeholder="Nick exato..."
                                    className="w-full bg-fmb-slate border border-fmb-army/30 focus:border-red-500 py-1.5 pl-6 pr-2 rounded text-white text-xs outline-none font-mono"
                                    required
                                  />
                                </div>
                              </div>
                              ...
                              <div className="md:col-span-1">
                                <label className="text-[10px] uppercase text-fmb-gold block mb-1">Quantidade</label>
                                <select
                                  value={warnQuantity}
                                  onChange={(e) => setWarnQuantity(Number(e.target.value))}
                                  className="w-full bg-fmb-slate border border-fmb-army/30 focus:border-red-500 py-1.5 px-2 rounded text-white text-xs outline-none cursor-pointer font-bold font-mono"
                                >
                                  <option value={1}>1 Advertência</option>
                                  <option value={2}>2 Advertências</option>
                                  <option value={3}>3 Advertências (Banimento Automático)</option>
                                </select>
                              </div>

                              <div className="md:col-span-1">
                                <label className="text-[10px] uppercase text-fmb-gold block mb-1">Ação Rápida</label>
                                <div className="text-[9px] text-gray-400 mt-2 font-mono">
                                  Será somado ao perfil do militar de forma cumulativa.
                                </div>
                              </div>
                            </div>

                            <div>
                              <label className="text-[10px] uppercase text-fmb-gold block mb-1">Motivo / Artigo da Infração Disciplinar</label>
                              <textarea
                                placeholder="Descreva de forma tática e clara o motivo disciplinar desta sanção..."
                                rows={2}
                                value={warnReason}
                                onChange={(e) => setWarnReason(e.target.value)}
                                className="w-full bg-fmb-slate border border-fmb-army/30 focus:border-red-500 py-1.5 px-2 rounded text-white text-xs outline-none resize-none font-mono"
                                required
                              />
                            </div>
                            ...
                            <button
                              ...
                              className="w-full bg-red-900 hover:bg-red-850 border border-red-500/40 py-2 text-white font-bold rounded text-xs uppercase tracking-wider transition-colors font-mono cursor-pointer"
                            >
                              {warningInProgress ? "APLICANDO SANÇÃO..." : "Consagrar Advertência Disciplinar"}
                            </button>
                          </form>
                        </div>
                      )}
