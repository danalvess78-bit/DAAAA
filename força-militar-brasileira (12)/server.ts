...
function getDecoratedUser(user: any) {
  ...
  let missaoCorreta = `[FMB] ${user.role}`;
  if (latestCourseName) {
    const tagPart = latestInstructorTag ? ` [${latestInstructorTag.toUpperCase()}]` : "";
    missaoCorreta = `[FMB] ${user.role}${tagPart} [${latestCourseName.toUpperCase()}]`;
  }
...
app.get("/api/admin/enlistments", authenticateToken, (req: AuthRequest, res: Response) => {
  if (!req.userRank || !dbOperations.hasPermission(req.userRank, "canEnlist")) {
...
app.post("/api/training-categories", authenticateToken, (req: AuthRequest, res: Response) => {
  ...
  const hasAccess = req.userRank === MilitaryRank.ADMSUPREMO || dbOperations.hasPermission(req.userRank!, "canManageCategories") || dbOperations.hasPermission(req.userRank!, "canAdminSystem");
  if (!hasAccess) {
...
app.post("/api/document-categories", authenticateToken, (req: AuthRequest, res: Response) => {
  ...
  const hasAccess = req.userRank === MilitaryRank.ADMSUPREMO || dbOperations.hasPermission(req.userRank!, "canManageCategories") || dbOperations.hasPermission(req.userRank!, "canManageDocs") || dbOperations.hasPermission(req.userRank!, "canAdminSystem");
  if (!hasAccess) {
...
app.get("/api/instructor-categories", authenticateToken, (req: AuthRequest, res: Response) => {
  res.json(dbOperations.getInstructorCategories());
});
...
app.post("/api/instructor-categories", authenticateToken, (req: AuthRequest, res: Response) => {
  ...
  try {
    const list = dbOperations.addInstructorCategory(category);
    res.status(201).json(list);
  } catch (err: any) {
    res.status(400).json({ error: err.message });
  }
...
app.get("/api/aman-categories", authenticateToken, (req: AuthRequest, res: Response) => {
  res.json(dbOperations.getAmanCategories());
});
...
app.post("/api/aman-categories", authenticateToken, (req: AuthRequest, res: Response) => {
  ...
  try {
    const list = dbOperations.addAmanCategory(category);
    res.status(201).json(list);
  } catch (err: any) {
    res.status(400).json({ error: err.message });
  }
...
app.get("/api/esao-categories", authenticateToken, (req: AuthRequest, res: Response) => {
  res.json(dbOperations.getEsaoCategories());
});
...
app.post("/api/esao-categories", authenticateToken, (req: AuthRequest, res: Response) => {
  ...
  try {
    const list = dbOperations.addEsaoCategory(category);
    res.status(201).json(list);
  } catch (err: any) {
    res.status(400).json({ error: err.message });
  }
