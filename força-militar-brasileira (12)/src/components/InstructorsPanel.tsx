...
import { User, PoliceDocument, SubCargo } from "../types.js";
...
export default function InstructorsPanel({ viewer }: InstructorsPanelProps) {
  ...
  const handleAddCategory = async () => {
    ...
    try {
      ...
      if (activeSection === "instrutores") {
        updatedCats = await api.addInstructorCategory(trimmed);
      } else if (activeSection === "aman") {
        updatedCats = await api.addAmanCategory(trimmed);
      } else if (activeSection === "esao") {
        updatedCats = await api.addEsaoCategory(trimmed);
      }
            ...
            <div className="bg-fmb-black/40 border border-fmb-army/30 p-4 rounded-lg text-left">
              <div className="border-b border-fmb-army/20 pb-2 mb-3 flex items-center justify-between">
                ...
                {viewer.instructorTag ? (
                  <span className="text-[9px] bg-fmb-gold text-fmb-black font-black font-mono px-1.5 py-0.5 rounded uppercase">
              ...
              <div className="space-y-3 leading-tight">
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 bg-fmb-black border border-fmb-army/30 rounded px-2 py-1 text-center font-mono text-xs text-white uppercase outline-none focus:border-fmb-gold"
                  />
                  <button
                    onClick={async () => {
          ...
          <div className="bg-fmb-black/40 border border-fmb-army/30 p-4 rounded-lg text-left">
            <div className="flex items-center justify-between border-b border-fmb-army/20 pb-2 mb-3">
              <h4 className="font-display font-extrabold text-xs text-white uppercase tracking-wider flex items-center gap-1.5">
                <span>📁</span> Subcategorias ({categories.length})
              </h4>
              <button
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          ...
          >
            <h3 className="font-display font-black text-sm text-white uppercase tracking-wider mb-2 flex items-center gap-1.5">
              <span>FolderPlus</span> Nova Subcategoria de {activeSection === "instrutores" ? "Instrutor" : "AMAN"}
            </h3>
            <p className="text-[10px] font-mono text-gray-400 mb-4 leading-normal">
              Crie uma pasta de subcategoria exclusiva para separar os documentos da área {activeSection.toUpperCase()}.
            </p>

            <div className="space-y-3 text-left">
              <label className="text-[9px] font-mono text-gray-500 block uppercase">Nome da Subcategoria</label>
              <input
                type="text"
